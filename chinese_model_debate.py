...
def create_chinese_debate_config():
    """创建中文辩论配置。"""
    ...
    config = prefab_lib.Config(
        default_premise="两位专家就'应对气候变化更好的解决方案是可再生能源还是核能？'这一话题进行正式辩论。辩论包括开场陈述、反驳和结语三个环节。请按照严格的{name} -- \"内容\"格式进行发言。",
        default_max_steps=10,  # 控制辩论轮数
        prefabs=prefabs,
        instances=instances,
    )
...
def run_chinese_debate():
    ...
    print("正在创建辩论配置...")
    config = create_chinese_debate_config()
