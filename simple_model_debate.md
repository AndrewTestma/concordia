...
这个示例展示了如何使用Concordia框架让两个不同的AI模型进行简单的对话辩论。
...
这个示例可以直接运行，无需任何API密钥，因为它使用了模拟模型：
...
如果您想使用真实的Qwen和DeepSeek模型，请执行以下步骤：
...
这个简化示例展示了Concordia框架如何让不同的AI模型进行结构化对话，可以用于研究模型间的交互、观点表达和辩论能力。
